...
public class MailUtils {
    ...
    public static boolean sendEmail(Set<String> emails, String title, String content) {
        ...
        try {
            ...
            message.setFrom(new InternetAddress(emailAccount, "zkm", "UTF-8"));
            ...
            log.error("qq有误请重新输入", e);
            throw new BusinessException(ErrorCode.PARAMS_ERROR,"请输入正确的qq邮箱");
        } catch (Exception e) {
            //todo 给一份详细的报错返回给前端，是发送失败还是qq号有误
            log.error("发送邮件失败,系统繁忙,请稍后再试！", e);
            return false;
        }
    }
